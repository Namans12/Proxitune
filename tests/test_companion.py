...
def test_phone_controller_switches_known_zone():
    ...
    try:
        ...
        connection.request("POST", "/zone?token=secret", body=json.dumps({"zone": "echo"}), headers={"Content-Type": "application/json"})
...
def test_phone_controller_forwards_media_action():
    ...
    try:
        connection = HTTPConnection("127.0.0.1", server.server_port)
        connection.request("POST", "/media?token=secret", body=json.dumps({"action": "next"}), headers={"Content-Type": "application/json"})
